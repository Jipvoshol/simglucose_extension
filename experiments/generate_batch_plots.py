...
def jitter_meals(base_meals: List[Tuple[datetime, int]], seed: int, max_min: int = 15, max_g: int = 15):
    rng = np.random.RandomState(seed)
    res = []
...
def run_simulation(cohort: str, seed: int, use_context: bool) -> Tuple[pd.DataFrame, List[Tuple[datetime, int]]]:
    ...
    if ctx is not None:
        env = ContextAwareT1DSimEnv(patient, sensor, pump, scenario,
                                    context_stream=ctx,
                                    modulate_vm0=True, modulate_vmx=True, modulate_p2u=True)
    else:
    ...
    for _ in range(480):
        ...
        rows.append({
            ...
            'insulin': info.get('action_basal', act.basal) + info.get('action_bolus', act.bolus),
    ...
    # IOB calculation
    def compute_iob(ins_rate_series: pd.Series, dt_min: float = 3.0, dia_min: float = 360.0) -> pd.Series:
        iob = []
        remain = 0.0
...
def plot_standard(cohort: str, seed: int, df_b: pd.DataFrame, df_c: pd.DataFrame, 
                  meals: List[Tuple[datetime, int]], out: Path):
    ...
    ax1.plot(df_b.index, df_b['glucose'], 'gray', linestyle='--', 
             label='Baseline (no context)', linewidth=2.5, alpha=0.7)
    ax1.plot(df_c.index, df_c['glucose'], 'purple', 
             label='Context-aware (delayed response)', linewidth=2.5, alpha=0.9)
    ...
    for t, g in meals:
        ...
        ax1.text(t, ax1.get_ylim()[1] * 0.97, f'{g}g', ha='center', fontsize=8, 
    ...
    ax1.set_ylim(40, 300)
    ax1.set_title(f'OpenAPS: Baseline vs Context-Aware — {cohort} seed={seed}', 
    ...
    ax2.fill_between(df_c.index, 1.0, df_c['m_t'], where=(df_c['m_t'] > 1.0),
                      alpha=0.3, color='orange', label='↑ Sensitivity (exercise)')
    ax2.fill_between(df_c.index, 1.0, df_c['m_t'], where=(df_c['m_t'] < 1.0),
                      alpha=0.3, color='red', label='↓ Sensitivity (stress)')
    ...
    ax4.plot(df_b.index, df_b['IOB'], 'gray', linestyle='--', 
             label='Baseline IOB', linewidth=2, alpha=0.7)
    ax4.plot(df_c.index, df_c['IOB'], 'purple', 
             label='Context IOB', linewidth=2, alpha=0.8)
    ...
    ax5.plot(df_b.index, df_b['basal'] * 60, 'gray', linestyle='--', 
             label='Baseline basal', linewidth=1.5, alpha=0.7)
    ax5.plot(df_c.index, df_c['basal'] * 60, 'purple', 
             label='Context basal', linewidth=1.5, alpha=0.8)
    ...
    if len(b_bolus) > 0:
        ax6.scatter(b_bolus.index, b_bolus['bolus'] * 3, color='gray', marker='o', 
                    s=50, label='Baseline SMB', alpha=0.7, edgecolors='black')
    if len(c_bolus) > 0:
        ax6.scatter(c_bolus.index, c_bolus['bolus'] * 3, color='purple', marker='s', 
                    s=50, label='Context SMB', alpha=0.8, edgecolors='darkviolet')
...
def plot_extended(cohort: str, seed: int, df_b: pd.DataFrame, df_c: pd.DataFrame, 
                  meals: List[Tuple[datetime, int]], out: Path):
    ...
    ax1.plot(df_c.index, df_c['glucose'], 'purple', 
             label='Context-aware', linewidth=2.5, alpha=0.9)
    ...
    ax1.grid(True, alpha=0.3)
    ax1.set_ylim(40, 300)
    ax1.set_title(f'OpenAPS: Baseline vs Context — {cohort} seed={seed} (Extended Analysis)', 
                  fontsize=14, fontweight='bold')

    # Panel 2: DELTA GLUCOSE (physiological effect)
    ax2 = fig.add_subplot(gs[1], sharex=ax1)
    delta_glucose = df_c['glucose'].values - df_b['glucose'].values
    ax2.fill_between(df_c.index, 0, delta_glucose, where=(delta_glucose < 0),
                      alpha=0.5, color='orange', label='Context LOWER (good for exercise)')
    ax2.fill_between(df_c.index, 0, delta_glucose, where=(delta_glucose > 0),
                      alpha=0.5, color='red', label='Context HIGHER')
    ...
    ax3.fill_between(df_c.index, 1.0, df_c['m_t'], where=(df_c['m_t'] > 1.0),
    ...
    ax6.fill_between(df_c.index, 0, delta_insulin, where=(delta_insulin < 0),
                      alpha=0.5, color='blue', label='Less insulin (context)')
    ax6.fill_between(df_c.index, 0, delta_insulin, where=(delta_insulin > 0),
                      alpha=0.5, color='red', label='More insulin (context)')
    ax6.axhline(0, color='black', linestyle='-', linewidth=1.5, alpha=0.7)
    ax6.set_ylabel('ΔInsulin\n(Context - Baseline)\n(U/hr)', fontsize=10, fontweight='bold')
    ax6.legend(loc='upper right', fontsize=9)
    ax6.grid(True, alpha=0.3)
    ax6.set_title('Controller Response: How context alters insulin delivery', fontsize=10, style='italic')
    ...
    c_bolus = df_c[df_c['bolus'] > 0.001]
    if len(b_bolus) > 0:
        ax8.scatter(b_bolus.index, b_bolus['bolus'] * 3, color='gray', marker='o', 
                    s=50, alpha=0.7, edgecolors='black')
    if len(c_bolus) > 0:
        ax8.scatter(c_bolus.index, c_bolus['bolus'] * 3, color='purple', marker='s', 
                    s=50, alpha=0.8, edgecolors='darkviolet')
