...
cfg = ContextConfig(
    alpha=1.25, beta=0.8, mmax=2.5, mmin=0.70,
    hr_rest=70.0, hr_max=180.0, 
...
def run_hybrid_simulation(use_context=False):
    ...
    for step_idx in range(480):  # 24 hours
        ...
        pid_action = pid_controller.policy(
            obs, reward=0, done=False,
            sample_time=3
        )
...
for p in periods:
    ...
    print(f"{p['name']:<25} {p['baseline_bg']:<15.1f} {p['context_bg']:<15.1f} {p['diff']:+.1f} {direction:<10} {p['m_t']:.2f}")
...
fig.suptitle('Hybrid Controller (PID Basal + Meal Bolus): Baseline vs Context-Aware\n(V2 + IOB: Complete Realistic Simulation)', 
...
ax1.plot(hours_base, df_baseline['glucose'], label='Baseline (no HR/stress)', 
        color=colors['baseline'], linewidth=2.5, alpha=0.9, linestyle='--')
ax1.plot(hours_ctx, df_context['glucose'], label='Context (with HR/stress)', 
        color=colors['context'], linewidth=2.5, alpha=0.9)
...
for i, (t, cho) in enumerate(zip(df_baseline.index, df_baseline['CHO'])):
    if cho > 0:
        ...
        if not meals_plotted or hour - meals_plotted[-1] > 0.5:
            ...
            ax1.text(hour, 245, f'{int(cho)}g', ha='center', va='bottom', 
...
ax1.set_title('A: Glucose - Hybrid Controller (PID basal + meal bolus)', 
              fontsize=12, loc='left', pad=10)

# Panel 2: Glucose difference
ax2 = fig.add_subplot(gs[1, 0])
glucose_diff = df_context['glucose'].values - df_baseline['glucose'].values
ax2.fill_between(hours_ctx, 0, glucose_diff, where=(glucose_diff < 0),
                 color=colors['exercise'], alpha=0.4, label='Context LOWER')
ax2.fill_between(hours_ctx, 0, glucose_diff, where=(glucose_diff > 0),
                 color=colors['stress'], alpha=0.4, label='Context HIGHER')
...
ax2.set_title('B: Physiological Effect', 
              fontsize=12, loc='left', pad=10)

# Panel 3: Insulin total
ax3 = fig.add_subplot(gs[2, 0])
ax3.plot(hours_base, df_baseline['insulin'], color=colors['baseline'], linewidth=2, alpha=0.7, 
        label='Baseline', linestyle='--')
ax3.plot(hours_ctx, df_context['insulin'], color=colors['context'], linewidth=2, alpha=0.7,
        label='Context')
ax3.set_ylabel('Insulin Total\n(U/min)', fontsize=11, fontweight='bold')
ax3.set_xlim(0, 24)
ax3.legend()
ax3.grid(True, alpha=0.3)
ax3.set_title('C: Insulin (Basal + Bolus)', 
              fontsize=12, loc='left', pad=10)

# Panel 4: IOB (Insulin On Board)
ax4 = fig.add_subplot(gs[3, 0])
ax4.plot(hours_base, df_baseline['IOB'], color=colors['baseline'], linewidth=2, alpha=0.7, 
        label='Baseline', linestyle='--')
ax4.plot(hours_ctx, df_context['IOB'], color=colors['context'], linewidth=2, alpha=0.7,
        label='Context')
...
ax4.set_title('D: Insulin On Board (DIA = 360 min)', 
              fontsize=12, loc='left', pad=10)

# Panel 5: Insulin difference
ax5 = fig.add_subplot(gs[4, 0])
insulin_diff = df_context['insulin'].values - df_baseline['insulin'].values
ax5.fill_between(hours_ctx, 0, insulin_diff, where=(insulin_diff < 0),
                 color='blue', alpha=0.3, label='Less insulin')
ax5.fill_between(hours_ctx, 0, insulin_diff, where=(insulin_diff > 0),
                 color='red', alpha=0.3, label='More insulin')
...
ax5.set_title('E: PID Responds to Altered Glucose Dynamics', 
              fontsize=12, loc='left', pad=10)

# Panel 6: m(t)
ax6 = fig.add_subplot(gs[5, 0])
ax6.plot(hours_ctx, df_context['m_t'], 'k-', linewidth=2.5)
ax6.fill_between(hours_ctx, 1, df_context['m_t'], where=(df_context['m_t'] > 1),
                 color=colors['exercise'], alpha=0.3, label='↑ Sensitivity (exercise)')
ax6.fill_between(hours_ctx, df_context['m_t'], 1, where=(df_context['m_t'] < 1),
                 color=colors['stress'], alpha=0.3, label='↓ Sensitivity (stress)')
...
ax6.set_title('F: Context Modulation (logged via step_result.info)', 
              fontsize=12, loc='left', pad=10)

# Panel 7: HR
ax7 = fig.add_subplot(gs[6, 0])
ax7.plot(hours_ctx, df_context['hr'], 'r-', linewidth=2, alpha=0.7, label='HR')
ax7.fill_between(hours_ctx, 70, df_context['hr'], where=(df_context['hr'] > 70),
                 color='red', alpha=0.2)
...
ax7.set_title('G: Heart Rate Input', 
              fontsize=12, loc='left', pad=10)
