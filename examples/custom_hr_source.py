...
def create_csv_example(csv_path):
    ...
    df = pd.DataFrame({"timestamp": idx, "hr": hr, "eda": eda})
    ...
    def simulate_stream(self, start_time, n_samples=50):
        ...
        for i in range(n_samples):
            ...
            if i % 10 == 0:
                ...
                print(
                    f"  {timestamp.strftime('%H:%M')} | Buffer size: {len(hr_series)} | "
                    f"HR: {hr:.0f} bpm"
                )
...
def main():
    ...
    hr_synth, eda_synth = generate_synthetic_context(
        start_time=datetime(2025, 1, 1, 6, 0), duration_hours=24
    )
    ...
    hr_stream, eda_stream = buffer.simulate_stream(
        start_time=datetime(2025, 1, 1, 6, 0), n_samples=50
    )
